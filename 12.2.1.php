<?php include "db.php";
function calculatePoints($policyCount) {
    return $policyCount;
 }
// Create
if (isset($_POST['create'])) {
    $policy = $_POST['ethical_sourcing_policy'];
    $title = $_POST['title_of_the_ppa'];
    $short_desc = $_POST['short_description_of_the_ppa'];
    $cost = $_POST['total_cost'];
    $fund = $_POST['fund_source'];

    if (empty($policy) || empty($title) || empty($short_desc) || !is_numeric($cost) || empty($fund)) {
        echo "<script type='text/javascript'>alert('All fields are required and total cost must be a number.');</script>";
    } else {
        $query = "INSERT INTO 12_2_1 (ethical_sourcing_policy, title_of_the_ppa, short_description_of_the_ppa, total_cost, fund_source) VALUES ( '$policy', '$title', '$short_desc', '$cost', '$fund')";
...
if (isset($_POST['update'])) {
    ...
    if (empty($policy) || empty($title) || empty($short_desc) || !is_numeric($cost) || empty($fund)) {
        ...
        $query = "UPDATE 12_2_1 SET ethical_sourcing_policy = '$policy' title_of_the_ppa='$title', short_description_of_the_ppa='$short_desc', total_cost='$cost', fund_source='$fund' WHERE id='$id'";
...
<body background="backgroundf.avif">
    <div class="container">
        ...
        <div class="form-container">
            ...
            <div>
                    <label for="ethical_sourcing_policy">Total number of ethical sourcing policy::</label>
                    <input type="text" name="ethical_sourcing_policy" id="ethical_sourcing_policy" required>
                </div>
                <div>
        ...
        <div class="table-container">
            ...
            <table>
                <thead>
                    <tr>
                        <th>Total number of ethical sourcing policy</th>
                        <th>Title</th>
                ...
                <tbody>
                    <?php
                    $query = "SELECT * FROM 12_2_1";
                    $result = mysqli_query($conn, $query);

                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $points = calculatePoints($row['ethical_sourcing_policy']);
                            echo "<tr>
                                     <td>{$row['ethical_sourcing_policy']}</td>
                                    ...
                                    <td class='action-buttons'>
                                        <form action='' method='post' style='display:inline;'>
                                            ...
                                            <input type='hidden' name='ethical_sourcing_policy' value='{$row['ethical_sourcing_policy']}'>
        ...
        echo "<div class='container'>
                ...
                <div class='form-container'>
                    <form action='' method='post'>
                        ...
                        <div>
                        <label for='ethical_sourcing_policy'>Total number of ethical sourcing policy::</label>
                         <input type='text' name='ethical_sourcing_policy' id='ethical_sourcing_policy' value='$hazardous' required>
                    </div>
                        <div>
                            <label for='title_of_the_ppa'>Title:</label>
