<?php
include "db.php";
function calculatePoints($extendedpolicy) {
    return $extendedpolicy;
 }
// Create
if (isset($_POST['create'])) {
    $suppliers = $_POST['disposable_policy'];
    $title = $_POST['title_of_the_ppa'];
    $short_desc = $_POST['short_description_of_the_ppa'];
    $cost = $_POST['total_cost'];
    $fund = $_POST['fund_source'];

    if (empty($suppliers) || empty($title) || empty($short_desc) || !is_numeric($cost) || empty($fund)) {
        echo "<script type='text/javascript'>alert('All fields are required and total cost must be a number.');</script>";
    } else {
        $query = "INSERT INTO 12_2_6 (disposable_policy, title_of_the_ppa,short_description_of_the_ppa, total_cost, fund_source) VALUES ('$suppliers','$title', '$short_desc', '$cost', '$fund')";
...
if (isset($_POST['update'])) {
    ...
    if (empty($suppliers) || empty($title) || empty($short_desc) || !is_numeric($cost) || empty($fund)) {
        ...
        $query = "INSERT INTO 12_2_2 (disposable_policy, title_of_the_ppa,short_description_of_the_ppa, total_cost, fund_source) VALUES ('$suppliers','$title', '$short_desc', '$cost', '$fund')";
        $add_research = mysqli_query($conn, $query);
...
if (isset($_POST['delete'])) {
    ...
    $query = "DELETE FROM 12_2_6 WHERE id='$id'";
...
<body background="backgroundf.avif">
    <div class="container">
        ...
        <div class="form-container">
            ...
            <div>
                    <label for="disposable_policy">Total number of disposable policy for external suppliers:</label>
                    <input type="text" name="disposable_policy" id="disposable_policy" required>
                </div>
                <div>
        ...
        <div class="table-container">
            ...
            <table>
                <thead>
                    <tr>
                        <th>Total number of disposable policy for external suppliers</th>
                        <th>Title</th>
                ...
                <tbody>
                    <?php
                    $query = "SELECT * FROM 12_2_6";
                    $result = mysqli_query($conn, $query);

                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $points = calculatePoints($row['disposable_policy']);
                            echo "<tr>
                                   <td>{$row['disposable_policy']}</td>
                                    ...
                                    <td class='action-buttons'>
                                        <form action='' method='post' style='display:inline;'>
                                            ...
                                            <input type='hidden' name='disposable_policy' value='{$row['disposable_policy']}'>
    ...
    if (isset($_POST['edit'])) {
        ...
        echo "<div class='container'>
                ...
                <div class='form-container'>
                    ...
                    <div>
                        <label for='disposable_policy'>Total number of disposable policy for external suppliers:</label>
                        <input type='text' name='disposable_policy' id='disposable_policy' value='$suppliers' required>
                    </div>
                        <div>
                            <label for='title_of_the_ppa'>Title:</label>
