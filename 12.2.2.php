<?php
include "db.php";
function calculatePoints($hazardousrisk) {
    return $hazardousrisk;
 }
// Create
if (isset($_POST['create'])) {
    $hazardous = $_POST['waste_disposal'];  
    $title = $_POST['title_of_the_ppa'];
    $short_desc = $_POST['short_description_of_the_ppa'];
    $cost = $_POST['total_cost'];
    $fund = $_POST['fund_source'];

    if (empty($hazardous) || empty($title) || empty($short_desc) || !is_numeric($cost) || empty($fund)) {
        echo "<script type='text/javascript'>alert('All fields are required and total cost must be a number.');</script>";
    } else {
        $query = "INSERT INTO 12_2_2 (waste_disposal, title_of_the_ppa, short_description_of_the_ppa, total_cost, fund_source) VALUES ('$hazardous','$title', '$short_desc', '$cost', '$fund')";
...
if (isset($_POST['update'])) {
    ...
    if (empty(empty($hazardous) || $title) || empty($short_desc) || !is_numeric($cost) || empty($fund)) {
        echo "<script type='text/javascript'>alert('All fields are required and total cost must be a number.');</script>";
    } else {
        $query = "UPDATE 12_2_2 SET waste_disposal = '$hazardous', title_of_the_ppa='$title', short_description_of_the_ppa='$short_desc', total_cost='$cost', fund_source='$fund' WHERE id='$id'";
...
if (isset($_POST['delete'])) {
    ...
    $query = "DELETE FROM 12_2_2 WHERE id='$id'";
...
<body background="backgroundf.avif">
    <div class="container">
        ...
        <div class="form-container">
            ...
            <div>
                    <label for="waste_disposal">Total number of waste disposal policy for hazardous waste:</label>
                    <input type="text" name="waste_disposal" id="waste_disposal" required>
                </div>
                <div>
        ...
        <div class="table-container">
            ...
            <table>
                <thead>
                    <tr>
                        <th>Total number of waste disposal policy for hazardous waste</th>
                        <th>Title</th>
                ...
                <tbody>
                    <?php
                    $query = "SELECT * FROM 12_2_2";
                    $result = mysqli_query($conn, $query);

                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $points = calculatePoints($row['waste_disposal']);
                            echo "<tr>
                                     <td>{$row['waste_disposal']}</td>
                                    ...
                                    <td class='action-buttons'>
                                        <form action='' method='post' style='display:inline;'>
                                            ...
                                            <input type='hidden' name='waste_disposal' value='{$row['waste_disposal']}'>
        ...
        echo "<div class='container'>
                ...
                <div class='form-container'>
                    <form action='' method='post'>
                        ...
                        <div>
                            <label for='waste_disposal'>Title:</label>
                            <input type='text' name='waste_disposal' id='waste_disposal' value='$hazardous' required>
                        </div>
                        <div>
                            <label for='title_of_the_ppa'>Title:</label>
