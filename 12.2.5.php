...
function calculatePoints($itemdisposal) {
    return $itemdisposal;
 }
// Create
if (isset($_POST['create'])) {
    $implemented = $_POST['ppa_implemented'];
    $title = $_POST['title_of_the_ppa'];
    $short_desc = $_POST['short_description_of_the_ppa'];
    $cost = $_POST['total_cost'];
    $fund = $_POST['fund_source'];

    if (empty($implemented) || empty($title) || empty($short_desc) || !is_numeric($cost) || empty($fund)) {
        echo "<script type='text/javascript'>alert('All fields are required and total cost must be a number.');</script>";
    } else {
        $query = "INSERT INTO 12_2_5 (ppa_implemented, title_of_the_ppa, short_description_of_the_ppa, total_cost, fund_source) VALUES ('$implemented', '$title', '$short_desc', '$cost', '$fund')";
...
if (isset($_POST['update'])) {
    ...
    if (empty($implemented) || empty($title) || empty($short_desc) || !is_numeric($cost) || empty($fund)) {
        ...
        $query = "UPDATE 12_2_5 SET ppa_implemented= '$implemented', title_of_the_ppa='$title', short_description_of_the_ppa='$short_desc', total_cost='$cost', fund_source='$fund' WHERE id='$id'";
...
if (isset($_POST['delete'])) {
    ...
    $query = "DELETE FROM 12_2_5 WHERE id='$id'";
...
<body background="backgroundf.avif">
    <div class="container">
        ...
        <div class="form-container">
            ...
            <div>
                    <label for="ppa_implemented">Total number of minimization policy for disposable items:</label>
                    <input type="text" name="ppa_implemented" id="ppa_implemented" required>
                </div>
                <div>
        ...
        <div class="table-container">
            ...
            <table>
                <thead>
                    <tr>
                        <th>Total number of minimization policy for disposable items</th>
                        <th>Title</th>
                ...
                <tbody>
                    <?php
                    $query = "SELECT * FROM 12_2_5";
                    $result = mysqli_query($conn, $query);

                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $points = calculatePoints($row['ppa_implemented']);
                            echo "<tr>
                                     <td>{$row['ppa_implemented']}</td>
                                    ...
                                    <td class='action-buttons'>
                                        <form action='' method='post' style='display:inline;'>
                                             <input type='hidden' name='ppa_implemented' value='{$row['ppa_implemented']}'>
                ...
                <div class='form-container'>
                    <form action='' method='post'>
                        <input type='hidden' name='id' value='$id'>
                         <div>
                            <label for='ppa_implemented'>Title:</label>
                            <input type='text' name='ppa_implemented' id='ppa_implemented' value='$implemented' required>
                        </div>
                        <div>
                            <label for='title_of_the_ppa'>Title:</label>
