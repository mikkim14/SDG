<?php
include "db.php";
function calculatePoints($plasticuse) {
    return $plasticuse;
 }
// Create
if (isset($_POST['create'])) {
    $plastic_use = $_POST['minimization_policy'];
    $title = $_POST['title_of_the_ppa'];
    $short_desc = $_POST['short_description_of_the_ppa'];
    $cost = $_POST['total_cost'];
    $fund = $_POST['fund_source'];

    if (empty($plastic_use) || empty($title) || empty($short_desc) || !is_numeric($cost) || empty($fund)) {
        echo "<script type='text/javascript'>alert('All fields are required and total cost must be a number.');</script>";
    } else {
        $query = "INSERT INTO 12_2_4 (minimization_policy, title_of_the_ppa, short_description_of_the_ppa, total_cost, fund_source) VALUES ('$plastic_use','$title', '$short_desc', '$cost', '$fund')";
...
if (isset($_POST['update'])) {
    ...
    if (empty($plastic_use) || ($title) || empty($short_desc) || !is_numeric($cost) || empty($fund)) {
        echo "<script type='text/javascript'>alert('All fields are required and total cost must be a number.');</script>";
    } else {
        $query = "UPDATE 12_2_4 SET minimization_policy='$plastic_use', title_of_the_ppa='$title', short_description_of_the_ppa='$short_desc', total_cost='$cost', fund_source='$fund' WHERE id='$id'";
...
if (isset($_POST['delete'])) {
    ...
    $query = "DELETE FROM 12_2_4 WHERE id='$id'";
    ...
    <div class="container">
        ...
        <div class="form-container">
            <form action="" method="post">
                <div>
                    <label for="minimization_policy">Total number of minimization policy for plastic use:</label>
                    <input type="text" name="minimization_policy" id="minimization_policy" required>
                </div>
                <div>
        ...
        <div class="table-container">
            ...
            <table>
                <thead>
                    <tr>
                        <th>Total number of minimization policy for plastic use</th>
                        <th>Title</th>
                ...
                <tbody>
                    <?php
                    $query = "SELECT * FROM 12_2_4";
                    $result = mysqli_query($conn, $query);

                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $points = calculatePoints($row['minimization_policy']);
                            echo "<tr>
                                     <td>{$row['minimization_policy']}</td>
                                    ...
                                    <td class='action-buttons'>
                                        <form action='' method='post' style='display:inline;'>
                                            ...
                                            <input type='hidden' name='minimization_policy' value='{$row['minimization_policy']}'>
        ...
        echo "<div class='container'>
                ...
                <div class='form-container'>
                    <form action='' method='post'>
                        ...
                        <div>
                        <label for='minimization_policy'>Total number of minimization policy for plastic use:</label>
                        <input type='text' name='minimization_policy' id='minimization_policy' value='$plastic_use' required>
                       </div>
                        <div>
                            <label for='title_of_the_ppa'>Title:</label>
